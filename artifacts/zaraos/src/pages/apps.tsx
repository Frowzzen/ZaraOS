...
import { Globe, FolderOpen, FileText, PlaySquare, Settings, ShieldAlert, Cpu, Code } from "lucide-react";
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {apps.map((app, index) => (
            <Link key={app.name} href={app.path}>
              <Card className={`cursor-pointer bg-card/40 backdrop-blur border-white/5 hover:${app.border} transition-all duration-300 group hover:-translate-y-1 hover:shadow-xl hover:shadow-cyan-900/10 animate-in fade-in slide-in-from-bottom-4`} style={{ animationDelay: `${index * 50}ms` }} data-testid={`app-tile-${app.name.toLowerCase()}`}>
